...
const lastWeek = new Date(Date.now()-604800000).toJSON()
...
function parsePost({ fields }) {
  return {
    ...
    youtube: fields.youtube ? fields.youtube : null,
...
export async function getAllPostsForHome(preview) {
  const entries = await getClient(preview).getEntries({
    content_type: "post",
    "sys.updatedAt[gte]": lastWeek,
    order: "-fields.date",
  });
  return parsePostEntries(entries);
}

export async function getAllPostsForArchive(preview) {
  const entries = await getClient(preview).getEntries({
    content_type: "post",
    "sys.updatedAt[lte]": lastWeek,
    order: "-fields.date",
  });
  return parsePostEntries(entries);
}
